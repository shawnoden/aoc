...
SET @inStr = REPLACE(REPLACE(@inStr,'/',''),'\','')
/* Replace do() and don't() */
SET @inStr = REPLACE(REPLACE(@inStr,'do()','/'),'dont()','\')
...
SELECT CASE WHEN TRY_PARSE(value AS numeric) IS NOT NULL THEN value ELSE NULL END, ordinal FROM STRING_SPLIT(@inStr,'|', 1)

DELETE FROM #tmpInstructions WHERE instr IS NULL
...
/* PART 2 */
-- x|2,4|&mul[3,7]!^\_|5,5|+|32,64](|11,8|un/?|8,5||
...
INSERT INTO #tmpInstructions2 (instr, doIT, n1, n2)
SELECT v, CASE WHEN action = 1 THEN 1 ELSE 0 END, 
	SUBSTRING(s2.nums,1,CHARINDEX(',',s2.nums)-1) AS n1, SUBSTRING(s2.nums,CHARINDEX(',',s2.nums)+1, 8) AS n2
FROM (
	SELECT s1.*, CASE WHEN TRY_PARSE(c2.value AS numeric) IS NOT NULL THEN c2.value ELSE null END AS nums
	FROM (
		SELECT * 
		FROM #tmpInstructions
		CROSS APPLY ( SELECT value AS v, ordinal AS action FROM STRING_SPLIT(instr,'\',1) ) c1
	) s1
	CROSS APPLY ( SELECT value FROM STRING_SPLIT(s1.v,'|') ) c2
) s2
WHERE nums IS NOT NULL

UPDATE #tmpInstructions2
SET total = CAST(n1 AS int) * cast(n2 AS int)
WHERE doIt = 1
...
/*
NOTES:
Because the input string had SQL special characters, specifically "'", I had to parse those out before I set the input.

Part 1:
This one was convoluted, but fairly simple. I just parsed out the needed two numbers into a temp table, then 
multiplied them together. 

First answer 196826776 was Correct! On to Part 2. 

===================================
Part 2:
Oh boy. More parsing. Now there's do() and don't() instructions. But since I removed the ' character, I hope I didn't break 
any "don't" by making them "dont", and that's one of the tricks. If so, I'll probably have to figure out how to escape them 
instead. 

OK, good. A quick scan through the input data doesn't show that trick. But they have caught on that people are using SQL to
parse these problems, so they've also inserted comment flags (/* and */) into the data. Guess I'll parse those out too. May
as well. They have also included almost every special character in this thing. This is a pain. 

-----

Again, after doing a little more cleanup on the input, I was able to parse out the pieces I needed. This is some ugly SQL,
but it works. 

First answer 106780429 was correct Day 3 is done!
