...
/* Instructions table. */
DROP TABLE IF EXISTS #troubleshootInstructions
...
; WITH base AS (
	...
	FROM (
		SELECT id, iLabel, iBoxID, iOperation, iFocalLength
			, ROW_NUMBER() OVER (PARTITION BY iLabel,iBoxID ORDER BY id DESC) AS rn
		FROM #tmpInstructions ti1
		WHERE iOperation = '='
	) s1
	WHERE s1.rn = 1
...
, guts AS (
	SELECT  iLabel, iBoxID, iOperation, iFocalLength
		, ROW_NUMBER() OVER ( ORDER BY id ) AS rn
	FROM (
		SELECT b.id, b.iLabel, b.iBoxID, b.iOperation, b.iFocalLength
			, ROW_NUMBER() OVER (PARTITION BY b.iLabel,b.iBoxID ORDER BY b.id DESC) AS rn
		FROM base b
		LEFT OUTER JOIN removes r ON b.ID > r.ID
			AND r.iLabel = b.iLabel
			AND r.iBoxID = b.iBoxID
	) s1
	WHERE rn = 1
)
--INSERT INTO #troubleshootInstructions (id, iLabel, iBoxID, iOperation, iFocalLength)
...
SET IDENTITY_INSERT #troubleshootInstructions OFF
...
SELECT boxRow, lensLabel, count(*) AS cnt
FROM #tmpBoxes
WHERE lensLabel IS NOT NULL
GROUP BY boxRow, lensLabel
ORDER BY cnt DESC
