...
DECLARE @inp varchar(max) = '8eight1
98126
...
DROP TABLE IF EXISTS #tmpString
CREATE TABLE #tmpString (instr varchar(max))

INSERT INTO #tmpString 
SELECT @inStr

--SELECT * FROM #tmpString

/**** PART 2 ONLY. REPLACE STRING NUMBERS ****/
UPDATE #tmpString
SET instr = 
	REPLACE(
		REPLACE(
...
/*
NOTES:
Advent of Code 2023 starts. I'm going to start these in SQL again, and it really hasn't gotten any 
better with string parsing, so we'll see how this goes. 

Part 1:
Looking at this one, it seemed super easy. I'll use the same algorithm I used last year to parse 
these inputs into table rows that I can work with. Then, for this one, all I need to do is run 
some regex on the input to strip out first and last digit characters. Easy peasy. 

Then I remembered that SQL Server is kinda shit at regex, especially in the SELECTs. So it 
became _MUCH_ more complicated. 

I first still tried to do it with some PATINDEX and some REGEX, but I couldn't get it to work
right. So I switched to just procedurally beating the data into submission. 

I put the full input into one single, really long string, then iterated over every character to
remove any non-digit character. And my seperator that I still needed to be able to spllit the 
lines into table rows. Then I just used LEFT() and RIGHT() to get the first and last character,
then just summed that up. 

Part 1 is done. 

Part 2:
Oh boy. Part 2 says that some of the digits are spelled out with letters. So now I have to parse
the strings to turn them into digits. Instructions say that I only have to check for 1-9, but 
the example shows "sixteen", which means I need to look at the actual input and see if there
are any like "twenty" or something. This could be a pain. Here we go...

OK. There don't seem to be any "twenty"s or other such madness, so I'll give it a shot just 
looking for 1-9. That'll be messy, but I should be able to use a set of REPLACEs...

It's ugly, but let's see...

-- Attempt 1: 54304 << WRONG

Crap. First answer too low. Where'd I mess up. Time to look at the data. 

Ugh. There are overlaps. Like "eightwo" or "oneight". Gotta handle those...

OK, changed the REPLACEs slightly to add leading and trailing characters on some of them, since
that should handle any character that's out of place. Not sure if there are some edge cases 
where the numbers are in an order that causes differences, but we'll see...

--Attempt 2: 54418 << CORRECT!

I should have known that these puzzles are never as straightforward as they look.
