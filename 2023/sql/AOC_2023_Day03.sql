...
DECLARE @inp varchar(max) = '...............776..............552........968..................589...26...........484..............958......186....546.........484.........
.........*.........778....................*....124...................................*...............*........%..26.........................
...
SET @inStr = 
	REPLACE(
		...
		,'@','#')
	,'&','#')
/* REPLACE ALL DELIMETERS WITH "#" EXCEPT "#" AND "*" *//* This is a very contrived list, pulled from final data */
...
CREATE TABLE #tmpRowsCols (id int identity, charVal varchar(1), rowNum int, colNum int, isNumber bit, fullNumber int  )
...
--	SELECT *
--		, CASE WHEN charVal LIKE '%[0-9]%' THEN 1 ELSE 0 END AS isNum
--		, TRY_PARSE(charVal AS int) AS isNum2
--		, CASE WHEN ( rowNum = LAG(rowNum) OVER (ORDER BY id) ) THEN LAG(charVal) OVER (ORDER BY id) ELSE NULL END AS previousChar
--	FROM #tmpRowsCols

--)
, fullNums AS (
	/* What if same sequence of numbers on line twice? */
	SELECT id, instr, strippedNum, CHARINDEX(strippedNum, instr) AS numPosStart, LEN(strippedNum) AS numLen, CHARINDEX(strippedNum, instr)+LEN(strippedNum)-1 AS numPosEnd
	FROM (
...
, numsOnRows AS (
	...
	INNER JOIN markers m ON (
		--m.rowNum BETWEEN rc.id-1 AND rc.id+1
		--AND m.colNum BETWEEN rc.numPosStart-1 AND rc.numPosEnd+1
		(m.rowNum BETWEEN rc.id-1 AND rc.id+1) AND ( m.colNum BETWEEN rc.numPosStart-1 AND rc.numPosEnd+1 )

	  	     --(m.colNum BETWEEN rc.numPosStart AND rc.numPosEnd AND rc.id=m.rowNum-1)
         --   OR (m.colNum BETWEEN rc.numPosStart AND rc.numPosEnd AND rc.id=m.rowNum+1)
         --   OR (m.colNum-1 BETWEEN rc.numPosStart AND rc.numPosEnd AND rc.id=m.rowNum)
         --   OR (m.colNum+1 BETWEEN rc.numPosStart AND rc.numPosEnd AND rc.id=m.rowNum)
...
/* Numbers / Markers on above/below row */


/* Check to see if any lines have numbers split by a marker other than * */
--SELECT * FROM #tmpInstructions WHERE instr LIKE '%[0-9][#][0-9]%'
...
/* 
Attempt 1: 4543606 <<< TOO HIGH.
Attempt 2: 666098 <<< TOO HIGH.
Attempt 3: 539741 <<< INCORRECT
...
Many days have passed (Day 20) and I'm finally back. Let's see if I can make any better progress. 

I've gone in a lot of circles about how to define numbers that are next to the markers. I've got a massive CTE that I think can 
identify them, but I do have a concern for part of it. It ran the test data fine, but I think it might mis-label some of these
numbers. Let's check. 
...
...
Found the error by looking at the initial data. My code isn't splitting two numbers when there is a "*" between them. I need
to fix that. 
ex: .....................306.........335.................*......+...*..................563..240..................*.......349*708...244...319....
...

ATTEMPT 2: 666098 <<< TOO HIGH

ARGH. Found a way to use multi-character-ish delimiters in STRING_SPLIT, but now my new answer is still too high. I'm still 
missing an edge case somewhere. 
...
I replaced all delemiters with "#" except for the "*", since I will need that one in Part 2 (looked ahead, sorry), and I will 
try again to split these columns correctly. 

I noticed a bug in my REGEX that I was using to pick out my markers. I was using [^1-9] instead of [^0-9], so I was picking 
up "0" as a marker. Oops. 

Running one more time. 

ATTEMPT 3: 539741 <<< TOO HIGH
...
I was getting duplicates in my final query before the sum, so some numbers were getting counted twice. This appeared 
to be happening when a marker landed on more than one character of a number. 

CRAP! Still not the right answer. I am really missing something here. 
...............
