...
/* PART 1 */
/*
SELECT 759-489 270+1
SELECT 964-959 5+1
SELECT 271*6 1626
*/


DECLARE @inp varchar(max) = 'turn on 489,959 through 759,964
...
SET @inp = REPLACE(@inp,'turn on ','1,') ;
SET @inp = REPLACE(@inp,'turn off ','0,') ;
SET @inp = REPLACE(@inp,'toggle ','2,') ;
SET @inp = REPLACE(@inp,' through ','.') ;

DECLARE @CRLF varchar(10) = char(13) + char(10) ;
DECLARE @inStr varchar(max) = REPLACE(@inp,@CRLF,'|')

DROP TABLE IF EXISTS #tmpDirections
CREATE TABLE #tmpDirections (instid int identity, instr varchar(200), actions int, startx int, starty int, endx int, endy int)

INSERT INTO #tmpDirections (instr, actions)
SELECT REPLACE(SUBSTRING(value,3,len(value)-2),',','.'),left(value,1) FROM STRING_SPLIT(@inStr,'|')

UPDATE #tmpDirections
SET   startx = PARSENAME(instr,4)
    , starty = PARSENAME(instr,3)
    , endx   = PARSENAME(instr,2)
    , endy   = PARSENAME(instr,1)


--SELECT * FROM #tmpDirections


/* CREATE MainGrid - 1000x1000 */

DROP TABLE IF EXISTS #mainGrid
CREATE TABLE #mainGrid (posx int, posy int, lightStatus bit DEFAULT 0)
...
BEGIN
    WHILE @c < 1000
    BEGIN
        INSERT INTO #mainGrid (posx, posy)
        VALUES ( @r, @c )
        SET @c += 1
    END
    SET @c=0
...
--SELECT * FROM #mainGrid ORDER BY posx,posy
/* RESET
UPDATE #mainGrid SET lightStatus = 0 
*/
...
DECLARE @maxRows int = (SELECT count(*) FROM #tmpDirections) 

WHILE @thisRow <= @MaxRows
BEGIN
   
    DECLARE @actions int = ( SELECT actions from #tmpDirections WHERE instid = @thisRow )
    DECLARE @rows int    = ( SELECT ((endy)-(starty)) FROM #tmpDirections WHERE instid = @thisRow )
    DECLARE @cols int    = ( SELECT ((endx)-(startx)) from #tmpDirections WHERE instid = @thisRow )
    DECLARE @startX int  = ( SELECT startx from #tmpDirections WHERE instid = @thisRow )
    DECLARE @starty int  = ( SELECT starty from #tmpDirections WHERE instid = @thisRow )

SELECT @actions, @rows, @cols, @startX, @starty, ((@cols+1)*(@rows+1)) AS updates

    UPDATE #mainGrid
    SET lightStatus = 
        CASE 
            WHEN @actions = 1 THEN 1
            WHEN @actions = 0 THEN 0
            WHEN @actions = 2 THEN CASE WHEN ISNULL(lightStatus,0) = 1 THEN 0 ELSE 1 END
        END
    WHERE 
        posx >= @startx AND posx <= @startx+@cols
        AND 
        posy >= @starty AND posy <= @starty+@rows

    SET @thisRow += 1
END

SELECT count(*) FROM #mainGrid WHERE lightStatus = 1

-- 530831 = too low
-- 531448 = too low
-- 571138 = too high
-- 549237 =  ??? Not correct
-- 569999 == CORRECT! Stupid NULLs
...
DROP TABLE IF EXISTS #mainGrid
CREATE TABLE #mainGrid (posx int, posy int, lightStatus int DEFAULT 0)
...
--SELECT @actions, @rows, @cols, @startX, @starty, ((@cols+1)*(@rows+1)) AS updates
    ...
    SET lightStatus = 
        CASE 
            WHEN @actions = 1 THEN lightStatus+1
            WHEN @actions = 0 THEN CASE WHEN lightStatus = 0 THEN 0 ELSE lightStatus-1 END
            WHEN @actions = 2 THEN lightStatus+2
        END
    WHERE 
        posx >= @startx AND posx <= @startx+@cols
        AND 
...
SELECT sum(lightStatus) FROM #mainGrid




-- 17325717 = Too Low
-- 34676064 = Too High - Didn't reset lights. Duh.
-- 17836115 = Correct.
...
Parse instructions to apply to grid. How many rows? How many cols?
1=on, 0=off, 2=toggle
Iterate instructions
This is horribly slow.
Lot's of Off-By-One errors in my calcs.
Stupid NULLs.

Part 2: 
Make sure lights can't go below 0.
Forgot to reset lights on final run.
...
DECLARE @thisRow int = 1
DECLARE @maxRows int = (SELECT count(*)-290 FROM #tmpDirections)
...
SELECT @actions, @rows AS r, @cols AS c, @startX AS startx, @starty AS starty, ((@cols+1)*(@rows+1)) AS updates
